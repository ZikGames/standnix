{self, inputs, options, lib, config, outputs, ...}: {
  ...
  flake.nixosModules.iso = {inputs, outputs, pkgs, ...}: {
  ...
  users.users.zik-iso = {
    ...
    description = "zik";
    extraGroups = [ "networkmanager" "pipewire" "wheel" "adbusers" "sudoers" "video" "audio" ];
    shell = pkgs.zsh;
  };
  ...
  isoImage.isoName = "labwc-nixos.iso";
  isoImage.squashfsCompression = "zstd";
  };

  flake.nixosModules.iso-hardware = { config, lib, pkgs, modulesPath, ... }: {
  ...
  # programs.neovim.enable = true;
   home.packages = with pkgs; [ steam-run nix-ld ];
